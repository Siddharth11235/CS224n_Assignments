...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for idx in range(window_size, len(labels)+1):
            word_feature = [k for j in range(idx -window_size,idx + window_size+1) for k in sentence[j]] 
            windowed_data.append((word_feature,labels[idx-1]))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup( params,
    self.input_placeholder)    
    ...
    def add_prediction_op(self):
        ...
        xavier_initializer = tf.contrib.layers.xavier_initializer(uniform=True,
            seed=None,
            dtype=tf.float32)
        W = tf.get_variable("W", shape=(Config.embed_size* Config.n_window_features,Config.hidden_size),dtype=tf.float32,
    initializer=xavier_initializer)
        U = tf.get_variable("U",shape =(Config.hidden_size, Config.n_classes),dtype=tf.float32,initializer=xavier_initializer)
